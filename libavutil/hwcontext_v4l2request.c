                                     ...
                                     AVDictionary *opts, int flags)
{
    ...
    hwctx->media_fd = open(device, O_RDWR, 0);
    if (hwctx->media_fd < 0)
...
static int v4l2request_device_init(AVHWDeviceContext *hwdev)
{
    ...
    av_log(hwdev, AV_LOG_VERBOSE, "Using V4L2 media driver %s (%d.%d.%d)\n",
           device_info.driver,
...
static int v4l2request_transfer_get_formats(AVHWFramesContext *ctx,
                                          ...
                                          AVFrame *dst, const AVFrame *src)
{
    ...
    err = v4l2request_map_frame(hwfc, map, src);
    if (err)
        goto fail;
