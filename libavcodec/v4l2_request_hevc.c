                               ...
                               const HEVCContext *h)
{
    ...
    *decode_params = (struct v4l2_ctrl_hevc_decode_params) {
    ...
    // AV1 and num_delta_pocs_of_ref_rps_idx where both added in v6.5-rc1
...
static int v4l2_request_hevc_set_controls(AVCodecContext *avctx)
{
    ...
    if (!ret) {
        ...
        if (ctx->decode_mode == V4L2_STATELESS_HEVC_DECODE_MODE_SLICE_BASED) {
            av_log(avctx, AV_LOG_ERROR, "%s: slice-based decoder, " \
        ...
        if (ctx->max_entry_point_offsets) {
            av_log(avctx, AV_LOG_ERROR,
                   "%s: decoder with ENTRY_POINT_OFFSETS control, " \
                   "required SLICE_PARAMS control is missing\n", __func__);
            return AVERROR(EINVAL);
        }
    }

    av_log(avctx, AV_LOG_DEBUG, "%s: %s-based decoder with SLICE_PARAMS=%u, " \
...
static int v4l2_request_hevc_init(AVCodecContext *avctx)
{
    ...
    // FIXME: use a better output buffer size

    ret = ff_v4l2_request_init(avctx, V4L2_PIX_FMT_HEVC_SLICE, 4 * 1024 * 1024,
                               control, FF_ARRAY_ELEMS(control));
    if (ret)
        return ret;

    // FIXME: check V4L2_CID_MPEG_VIDEO_HEVC_PROFILE control
    // FIXME: check V4L2_CID_MPEG_VIDEO_HEVC_LEVEL control
