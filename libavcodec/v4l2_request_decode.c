                                 ...
                                 struct v4l2_ext_control *control, int count)
{
    ...
    if (ret < 0) {
        av_log(ctx, AV_LOG_ERROR, "%s: set controls failed, %s (%d)\n",
                                     ...
                                     V4L2RequestBuffer *buf, uint32_t flags)
{
    ...
    // Queue the buffer
    ret = ioctl(ctx->video_fd, VIDIOC_QBUF, &buffer);
    if (ret < 0)
        return AVERROR(errno);
...
fail:
    ff_mutex_unlock(&ctx->mutex);
    av_log(ctx, AV_LOG_ERROR, "%s: waiting for capture buffer failed\n", __func__);
...
fail:
    ff_mutex_unlock(&ctx->mutex);
    av_log(ctx, AV_LOG_ERROR, "%s: waiting for output buffer failed\n", __func__);
    return NULL;
}
                                        ...
                                        V4L2RequestBuffer *output)
{
    ...
    ret = ioctl(output->fd, MEDIA_REQUEST_IOC_REINIT, NULL);
    if (ret < 0) {
        av_log(ctx, AV_LOG_ERROR, "%s: reinit request %d failed, %s (%d)\n",
               __func__, output->fd, strerror(errno), errno);
        return AVERROR(errno);
    }
                                  ...
                                  const uint8_t *data, uint32_t size)
{
    ...
    if (pic->output->used + size + INPUT_BUFFER_PADDING_SIZE <= pic->output->size) {
        ...
        av_log(ctx, AV_LOG_ERROR, "%s: output.index=%d used=%u size=%u size=%u\n",
               __func__, pic->output->index, pic->output->used, pic->output->size, size);
    }
                                     ...
                                     bool first_slice, bool last_slice)
{
    ...
    if (first_slice) {
         ...
         * V4L2 decoder may use a different capture buffer than FFmpeg expects.
         *
    ...
    ret = v4l2_request_set_controls(ctx, pic->output->fd, control, count);
    if (ret < 0) {
        av_log(ctx, AV_LOG_ERROR, "%s: set %d controls for request %d failed, %s (%d)\n",
               __func__, count, pic->output->fd, strerror(errno), errno);
        ret = AVERROR(errno);
        goto fail;
    }
    ...
    if (ret < 0) {
        av_log(ctx, AV_LOG_ERROR, "%s: queue output buffer %d for request %d failed, %s (%d)\n",
               __func__, pic->output->index, pic->output->fd, strerror(errno), errno);
        ret = AVERROR(errno);
        goto fail;
    }
    ...
    if (first_slice) {
        ...
        if (ret < 0) {
            av_log(ctx, AV_LOG_ERROR, "%s: queue capture buffer %d for request %d failed, %s (%d)\n",
                   __func__, pic->capture->index, pic->output->fd, strerror(errno), errno);
            ret = AVERROR(errno);
            goto fail;
        }
    }
    ...
    if (ret < 0) {
        av_log(ctx, AV_LOG_ERROR, "%s: queue request %d failed, %s (%d)\n",
...
fail:
    ff_mutex_unlock(&ctx->mutex);
    av_log(ctx, AV_LOG_ERROR, "%s: queueing request %d failed\n",
...
int ff_v4l2_request_start_frame(AVCodecContext *avctx, V4L2RequestPictureContext *pic, AVFrame *frame)
{
    ...
    // Prepare drm frame descriptor for current frame
    return ff_v4l2_request_set_drm_descriptor(desc, &ctx->format);
}
